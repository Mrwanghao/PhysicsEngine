...
bool Camera::IsOrthoNormal()
{
	...
	if (!compare(right.Dot(right), 1.0f) || !compare(up.Dot(up), 1.0f) || !compare(forward.Dot(forward), 1.0f)) 
	{
		return false; 
	}

	if (!compare(forward.Dot(up), 0.0f) || !compare(forward.Dot(right), 0.0f) || !compare(right.Dot(up), 0.0f)) 
	{
		return false;
	}
	return true;
...
//产生一个透视投影矩阵
void Camera::Perspective(float _fov, float _aspect, float _zNear, float _zFar)
{
	fov = _fov;
	aspect = _aspect;
	near = _zNear;
...
//产生一个正交投影矩阵
void Camera::Orthographic(float _width, float _height, float _zNear, float _zFar)
{
	width = _width;
	height = _height;
	near = _zNear;
...
Frustum Camera::GetFrustum()
{
	...
	Vec3 col1(vp.m00, vp.m10, vp.m20);
	Vec3 col2(vp.m01, vp.m11, vp.m21);
	Vec3 col3(vp.m02, vp.m12, vp.m22);
	Vec3 col4(vp.m03, vp.m13, vp.m23);
